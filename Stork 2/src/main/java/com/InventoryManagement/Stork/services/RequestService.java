    ...
    public static void updateRequest(Requests requestIn, int quantity) {
        ...
        for (Requests request : requests) {
            if (request.getRequestId() == requestIn.getRequestId()) {
                if(request.getAmountFilled() > quantity){
                    return;
                }
                request.setFullAmountRequested(quantity);
        ...
        for (Requests request : requests) {
            if(request.getRequestId() == fillRequest.getRequestId()){
                if(request.getAmountToBeFilled() > amount){
                    ...
                    List<Requests> requestListOfThisId = RequestService.getAllRequests(request.getRequestId(), requests);
                    ...
                    int tempAmount = 0;
                    for (Requests requestCurrent: requestListOfThisId){
                        tempAmount += requestCurrent.getAmountFilled();
                        RequestService.deleteRequestById(requestCurrent.getRequestId());
                    }
